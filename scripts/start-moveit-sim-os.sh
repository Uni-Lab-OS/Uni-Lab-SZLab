...
runtime_dir="${UNILAB_SZLAB_RUNTIME_DIR:-${repo_root}/runtime/moveit-sim}"
...
exec "${unilab_python}" -m unilabos.app.main \
  --workspace "${repo_root}" \
  --graph "${repo_root}/deployment/graphs/szlab-moveit-sim.json" \
  --config "${repo_root}/deployment/local_config.py" \
  --working_dir "${runtime_dir}" \
  --backend ros \
  --visual "${UNILAB_VISUAL:-disable}" \
  --app_bridges fastapi \
  --port "${UNILAB_OS_PORT:-18004}" \
  --disable_browser \
  --skip_env_check
