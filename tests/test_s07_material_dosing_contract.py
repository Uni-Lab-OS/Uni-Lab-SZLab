...
def test_s07_material_dosing_is_published_by_the_package(repo_root: Path) -> None:
    ...
    entries = {
        item["workflow_uuid"]: item["source"]
        for item in package["workflows"]
    }
...
def test_s07_deployment_warehouses_and_sites_are_not_workflow_inputs(
    ...
    argument_names = {
        argument.arg
        for argument in (*workflow.args.args, *workflow.args.kwonlyargs)
    }
...
def test_s07_material_dosing_has_two_parallel_material_branches_and_one_join(
    ...
    material_sources = [
        ...
        if isinstance(node, ast.Call) and _call_name(node) == "material_source"
    ]
    ...
    dose_calls = [
        ...
        and _call_name(node) == "dose_powder_with_materials"
    ]
