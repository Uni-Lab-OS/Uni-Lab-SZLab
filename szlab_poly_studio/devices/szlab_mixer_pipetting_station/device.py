...
class SzlabMixerPipettingStationDevice(UnifiedPLCGatewayMixin):
    def __init__(
        ...
        csv_path: str | None = "s09_pipetting_station/pipetting_station_nodes.csv",
        timeout: float = 300.0,
    ...
    def _wait_material_conditions(
        ...
        *,
        ...
        if callable(waiter):
            ...
            values: dict[str, Any] = {}
            while True:
                values = {name: self._read_variable(name, use_cache=False) for name in conditions}
    ...
    def _append_log(
        ...
        logs.append({"message": message, "detail": normalized_detail})
        logger.info(
            f"[SZLAB-STEP] {current_action_log_context()} "
            f"description={message} detail={compact_log_value(normalized_detail)}"
        )
    ...
    def _ensure_sufficient_remaining_volume(
        ...
        if current + 1e-9 < needed_ml:
            raise ValueError(f"S09 液体瓶 {bottle} 剩余液量不足：当前 {current:g} mL，需要 {needed_ml:g} mL")
        return {
            "bottle": bottle,
    ...
    def _deduct_remaining_volume(self, bottle: int, raw_volume: int) -> dict[str, Any]:
        ...
        if new_volume < 0:
            raise ValueError(f"S09 液体瓶 {bottle} 剩余液量不足：当前 {current:g} mL，需要 {deduct_ml:g} mL")
    ...
    @action(description="读取 S09 指定安全位原点信号")
    ...
    @action(description="读取 S09 四个安全位原点信号")
    ...
    @action(description="执行 S09 去安全位工艺并确认原点信号")
    ...
    @action(description="确认 S09 唯一加液工位空闲")
    ...
    @action(description="读取 S09 允许加工（允许参数写入）信号")
    ...
    @action(description="绑定样品到 S09 加液工位（占位）")
    ...
    @action(description="释放 S09 加液工位绑定（占位）")
    ...
    @action(description="执行 S09 单个 PLC 工艺")
    def run_process(
        self,
        process: int = 5,
        tip_box_index: int = 1,
        ...
        try:
            ...
            return {"success": False, "message": f"S09 工艺 {process} 前置物料传感器读取失败: {exc}"}
        ...
        try:
            ...
            should_read_balance = (
                process in {9, 10} if read_balance_after_done is None else bool(read_balance_after_done)
            )
            if should_read_balance:
                self._append_log(logs, "等待并读取 S09 稳定天平读数", {"process": process})
                if not self._wait_equal(S09_BALANCE_STABLE_VAR, True):
                    self._status = "Error"
                    return {"success": False, "message": "S09 天平稳定信号等待超时", "data": data, "logs": logs}
    ...
    @action(description="执行 S09 单次业务加液流程")
    def add_liquid(
        self,
        ...
        if tip_box_index is not None:
            take_tip_box_index = release_tip_box_index = tip_box_index
        try:
        ...
        for index, (aspirate_chunk, dispense_chunk) in enumerate(transfer_chunks, start=1):
            ...
            plan.append((7, f"液体瓶取液{suffix}", take_tip_box_index, aspirate_chunk, 0, False))
            plan.append((8, f"烧杯放液{suffix}", take_tip_box_index, 0, dispense_chunk, index == len(transfer_chunks)))
        plan.append((6, f"向 TIP盒{release_tip_box_index} 放 TIP", release_tip_box_index, 0, 0, False))

        for process, step_name, process_tip_box, aspirate_chunk, dispense_chunk, read_balance_after_done in plan:
            result = self.run_process(
                process=process,
                tip_box_index=process_tip_box,
                tip_index=tip_index,
                liquid_bottle_index=liquid_bottle_index,
                station=station,
        ...
        try:
            for index, item in enumerate(liquid_steps, start=1):
                ...
                if not result.get("success", False):
                    return {
                        "success": False,
                        "message": result.get("message", f"第 {index} 步加液失败"),
                        "steps": steps,
                    }
            return {
                "success": True,
    ...
    @action(description="写入 S09 单个液体瓶剩余液量")
    ...
    @action(description="初始化 S09 1-5 号液体瓶剩余液量")
    def initialize_liquid_bottle_remaining_volumes(self, remaining_volume: float = 100.0) -> dict[str, Any]:
        ...
        for bottle in range(1, 6):
            ...
            if not result.get("success", False):
                return {
                    "success": False,
                    "message": result.get("message", "写入剩余液量失败"),
                    "data": {"writes": writes},
                }
        return {
            "success": True,
    ...
    @action(description="读取 S09 天平读数")
    ...
    @action(description="读取 S09 移液站状态")
...
install_action_logging(SzlabMixerPipettingStationDevice)
