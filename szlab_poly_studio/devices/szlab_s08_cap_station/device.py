...
class SZLabS08CapStationDevice(UnifiedPLCGatewayMixin):
    ...
    def _wait_plc_bool(
        ...
        logger.info(
            f"[SZLAB-STEP] WAIT {context} description={desc} "
        ...
        try:
            actual = self._read_variable(node_name)
        except Exception as exc:
            actual = f"<read failed: {type(exc).__name__}: {exc}>"
        if completed:
            logger.info(
                f"[SZLAB-STEP] WAIT-SUCCESS {context} description={desc} "
                f"variable={node_name} expected={expected!r} "
                f"actual={compact_log_value(actual)}"
            )
        else:
            logger.error(
                f"[SZLAB-STEP] WAIT-TIMEOUT {context} description={desc} "
                f"variable={node_name} expected={expected!r} "
                f"last_actual={compact_log_value(actual)} timeout={float(timeout):.3f}s"
            )
        return completed
    ...
    def _wait_process_complete(
        ...
        if hasattr(self._client, "wait_equal"):
            ...
            if ok:
                ...
                logger.error(
                    f"✗ 等待 {desc} 超时 ({timeout}s)，最后读取 {var_ref}={self._last_process_complete_seen!r}"
                )
            return ok
        ...
        logger.error(f"✗ 等待 {desc} 超时 ({timeout}s)，最后读取 {var_ref}={last_seen!r}")
    ...
    def _process_complete_timeout_message(self, expected: int, task_label: str) -> str:
        ...
        return f"S08 {task_label} 等待工艺完成超时（期望 {var_ref}={expected}{suffix}）"
    ...
    def _read_sample_id_from_plc(self, slot: int) -> list[int]:
        return [
            int(self._read_variable(_cap_cache_element_name(slot, index)) or 0) for index in range(CAP_CACHE_LENGTH)
        ]
    ...
    def _validate_cap_station_has_bottle(self, vial_type: str, operation: str) -> None:
        ...
        if not bool(self._read_variable(sensor_node)):
            ...
            raise ValueError(f"{op_label}前检测到开盖工位{station_id}无{vial_label}（{sensor_node}=False）")
    ...
    def _resolve_open_cap_storage_slot(self, sample_id: Sequence[int]) -> int:
        ...
        if existing_slot is not None:
            if self._read_cap_slot_sensor(existing_slot):
                raise ValueError(f"该样品已开盖，瓶盖位于暂存位{existing_slot}，不能对同一瓶重复开盖")
            raise ValueError(
    ...
    def _run_cap_process(
        ...
        if self.require_station_ready:
            if not self._wait_plc_bool(NODE_HOME, True, timeout=timeout, description="S08 原点信号（机械臂安全位）"):
                return {
                    "success": False,
                    "message": (f"机械臂未回到 S08 安全位：{self._format_opc_variable_ref(NODE_HOME)} 当前为 False"),
                }
    ...
    @action(description="S08 开/关盖工艺的 Python 工作流标量参数入口")
    def process_cap_with_sample_parts(
        self,
        operation: str,
        vial_type: str,
        sample_id_1: int,
        ...
        """把 Python AST v1 支持的标量参数无损转换为 PLC sample_id 数组。"""
...
install_action_logging(SZLabS08CapStationDevice)
