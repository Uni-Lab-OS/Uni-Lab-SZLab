...
class S1Workstation(WorkstationBase):
    ...
    def _request(
        ...
        *,
        ...
        logger.info(
            f"[SZLAB-HTTP] START {context} method={method.upper()} url={url} "
            f"timeout={self.timeout:.3f}s params={compact_log_value(params)} "
            f"data={compact_log_value(data)}"
        )
        try:
            ...
            logger.error(
                f"[SZLAB-HTTP] FAIL {context} method={method.upper()} url={url} "
        ...
        log_method(
            f"[SZLAB-HTTP] {'SUCCESS' if success else 'FAIL'} {context} "
            f"method={method.upper()} url={url} "
            f"elapsed={time.monotonic() - started_at:.3f}s "
            f"code={response.get('code')!r} message={compact_log_value(self._message(response))}"
        )
...
install_action_logging(S1Workstation)
