...
class SzlabMixerOpcUaClient:
    def __init__(
        ...
        if auto_connect:
            ...
            self._nodes_by_name = {name: self.client.get_node(node_id) for name, node_id in self._node_id_map.items()}
    ...
    def _browse_device_nodes(self) -> dict[str, Any]:
        ...
        if not nodes:
            raise RuntimeError(
                "OPC UA 中未找到 VirtualMixer 对象，也没有递归扫描到可用变量节点；请确认真机 OPC UA 变量是否已发布"
            )
        return nodes
