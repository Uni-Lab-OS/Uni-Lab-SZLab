...
from ai4c_robot.opcua import OpcUaClientWithSubscription
...
class AI4CPLCDevice(OpcUaClientWithSubscription):
    ...
    @action(description="启动 AI4C PLC 心跳")
    ...
    @action(description="停止 AI4C PLC 心跳")
    ...
    @action(description="获取指定 PLC 变量的状态")
    ...
    def loading_rack_occupied(self) -> dict:
        return {
            str(position): bool(self.read_variable(f"Well_Plate_Loading_Rack_InPut[{position - 1}]", use_cache=True))
