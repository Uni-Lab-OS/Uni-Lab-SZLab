...
设备包应拥有设备驱动、动作、资源定义、工作流、Profile、协议表和模型资产；Uni-Lab-OS
...
5. 将 Profile 及其 device spec 放到 `profiles/<profile_id>/`，使用包内相对路径；
6. 将 CSV、JSON、Xacro、mesh、texture 放到拥有者目录；
7. 去除 OS 私有绝对 import、硬编码本机路径、凭据和运行数据；
8. 配置 package-data，构建 clean wheel 并验证静态发现结果；
9. 只有 Graph/Profile 选中设备后，运行时才允许 import 和连接硬件。
...
│   ├── profiles/default/          # package.yaml + device.yaml
│   ├── common/                    # 本包内部共享实现
│   └── __init__.py
├── deployment/graphs/             # 实例图；不属于装饰器扫描根
├── tests/                          # Registry、Profile、工作流、资产和 wheel 测试
...
- Profile：一组业务宏、资源拓扑和默认 binding，不自动激活硬件；
...
import 驱动、不实例化设备、不连接 PLC。Graph/Profile 只选择本次运行需要的设备；选中后才
加载对应 Python 类。

当前 OS 尚未实现完整 `--workspace`，所以仓库脚本暂时使用兼容入口：
...
workflow；不使用 `eval/exec`。Profile 声明宏动作和资源要求，Graph 决定具体实例。两者都消费
与设备/资源相同的 PackageCatalog，local bridge 不维护另一套扫描器。
...
验收至少覆盖：唯一根包、装饰器发现数量、Profile 自包含、工作流编译、模型相对路径、wheel
资产闭包、无跨包 import、默认不连硬件。OS delivery 完成后，再增加 workspace Catalog 与
clean-wheel Catalog 完全一致的跨仓 E2E。
